...
{
	for (int i=0; i<SERIALCOMMANDBUFFER_SIZE; i++)
	{
		buffer[i]='\0';
	}
	bufPos=0;
...
// Retrieve the next token ("word" or "argument") from the Command buffer.
// returns a NULL if no more tokens exist.
char *SerialCommand::next(char sep)
{
	char *nextToken;
	char tmpDelim[2];
	...
	while (Serial.available() > 0)
	{
		...
		{     // Check for the terminator (default '\r') meaning end of command
			...
			for (i=0; i<numCommand; i++)
			{
				...
				if (strncmp(token,CommandList[i].command,SERIALCOMMANDBUFFER_SIZE) == 0)
				{
					#ifdef SERIALCOMMANDDEBUG
		...
		if (isprint(inChar))   // Only printable characters into the buffer
		{
			...
			if (bufPos > SERIALCOMMANDBUFFER_SIZE-1) bufPos=0; // wrap buffer around if full
		}
	}
}
...
void SerialCommand::addCommand(const char *command, void (*function)())
{
	if (numCommand < MAXSERIALCOMMANDS) {
		...
		strncpy(CommandList[numCommand].command,command,SERIALCOMMANDBUFFER_SIZE);
